...
class AddAccessTokenIdToWebPushSubscriptions < ActiveRecord::Migration[5.2]
  ...
  def change
    add_reference :web_push_subscriptions, :access_token, null: true, default: nil, index: {algorithm: :concurrently}
    add_reference :web_push_subscriptions, :user, null: true, default: nil, index: {algorithm: :concurrently}
  end
end
