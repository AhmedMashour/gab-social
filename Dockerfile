...
RUN /bin/bash -l -c "export RAILS_SERVE_STATIC_FILES=true && export RAILS_ENV=production && export OTP_SECRET=precompile_placeholder && export SECRET_KEY_BASE=precompile_placeholder && export NODE_MODULES_CACHE=true && export -p && rails assets:precompile && yarn cache clean"
ENTRYPOINT ["/tini", "--"]
